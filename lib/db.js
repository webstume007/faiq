...
export async function getActivityLog(limit = 50, filters = {}) {
  ...
  if (filters.searchQuery) query = query.ilike('description', `%${filters.searchQuery}%`);
...
/** Update guardian (Admin edit) */
export async function updateGuardian(guardianId, userId, userUpdates, guardianUpdates, actorId = null) {
  if (Object.keys(userUpdates).length > 0) {
    await supabase.from('users').update(userUpdates).eq('id', userId);
  }
  if (Object.keys(guardianUpdates).length > 0) {
    await supabase.from('guardians').update(guardianUpdates).eq('id', guardianId);
  }
  if (actorId) {
    await logActivity(actorId, 'update', 'guardian', guardianId, null, { userUpdates, guardianUpdates }, `Updated guardian details for ID ${guardianId}`);
  }
  return { success: true };
}
...
export async function createSession(sessionData, actorId = null) {
  ...
  // Handle programs and classes if provided from Wizard
  if (sessionData.programs && sessionData.programs.length > 0) {
    const courseIdMap = {};
    for (const p of sessionData.programs) {
      const { data: newCourse } = await supabase
        ...
        .select()
        .single();
      if (newCourse && p.tempId) {
        courseIdMap[p.tempId] = newCourse.id;
      }
    }

    if (sessionData.classes && sessionData.classes.length > 0) {
      for (const c of sessionData.classes) {
        await supabase
          .from('session_classes')
          .insert({
            session_id: data.id,
            course_id: courseIdMap[c.courseTempId] || null,
...
// --- Dynamic Programs (Courses) ---

export async function updateCourse(courseId, updates, actorId = null) {
  const { data, error } = await supabase.from('session_courses').update(updates).eq('id', courseId).select().single();
  if (error) return { error: error.message };
  if (actorId) await logActivity(actorId, 'update', 'course', courseId, null, data, `Updated program ${data.course_name}`);
  return { course: data };
}

export async function deleteCourse(courseId, actorId = null) {
  const { error } = await supabase.from('session_courses').delete().eq('id', courseId);
  if (error) return { error: error.message };
  if (actorId) await logActivity(actorId, 'delete', 'course', courseId, null, null, `Deleted program`);
  return { success: true };
}

// --- Dynamic Classes ---

export async function updateClass(classId, updates, actorId = null) {
  const { data, error } = await supabase.from('session_classes').update(updates).eq('id', classId).select().single();
  if (error) return { error: error.message };
  if (actorId) await logActivity(actorId, 'update', 'class', classId, null, data, `Updated class ${data.class_name}`);
  return { classData: data };
}

export async function deleteClass(classId, actorId = null) {
  const { error } = await supabase.from('session_classes').delete().eq('id', classId);
  if (error) return { error: error.message };
  if (actorId) await logActivity(actorId, 'delete', 'class', classId, null, null, `Deleted class`);
  return { success: true };
}
...
/** Delete session */
export async function deleteSession(sessionId, actorId = null) {
  const { data: oldData } = await supabase.from('sessions').select('*').eq('id', sessionId).single();
  const { error } = await supabase.from('sessions').delete().eq('id', sessionId);
  if (error) return { error: error.message };

  if (actorId) {
    await logActivity(actorId, 'delete', 'session', sessionId, oldData, null, `Deleted session ${oldData?.name}`);
  }
...
export async function createAdmission(admissionData) {
  ...
  // Auto-generate Admission/Prospectus Challan
  const { challan, error: challanError } = await createChallan({
    application_id: data.id,
    session_id: admissionData.session_id,
    type: 'admission_fee',
    amount: 2000,
  ...
  return { admission: data, challan, challanError };
}
...
/** Update challan (Admin edit) */
export async function updateChallan(challanId, updates, actorId = null) {
  const { data: oldData } = await supabase.from('challans').select('*').eq('id', challanId).single();
  const { data, error } = await supabase
    .from('challans')
    .update(updates)
    .eq('id', challanId)
  ...
  if (actorId) {
    await logActivity(actorId, 'update', 'challan', challanId, oldData, data, `Updated challan ${oldData.challan_no}`);
  }
  return { challan: data };
}
