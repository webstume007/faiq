...
export async function createCourse(courseData, actorId = null) {
  const { data, error } = await supabase.from('session_courses').insert(courseData).select().single();
  if (error) return { error: error.message };
  if (actorId) await logActivity(actorId, 'create', 'course', data.id, null, data, `Added program ${data.course_name}`);
  return { course: data };
}
...
export async function createClass(classData, actorId = null) {
  const { data, error } = await supabase.from('session_classes').insert(classData).select().single();
  if (error) return { error: error.message };
  if (actorId) await logActivity(actorId, 'create', 'class', data.id, null, data, `Added class ${data.class_name}`);
